...
from trainer.utils import get_grad_norm, get_parameter_norm
...
class MaximumLikelihoodEstimationEngine(Engine):
    ...
    def __init__(self, func, model, crit, optimizer, lr_scheduler, config, device):
        self.model = model
        self.crit = crit
    ...
    @staticmethod
    def data_controller(engine, mini_batch):
    ...
    def train(engine, mini_batch):
        ...
        x, y = mini_batch[0], mini_batch[1][0][:, 1:]
        # |x| = (batch_size, length)
        # |y| = (batch_size, length)
        ...
        if torch.cuda.device_count() >= 1 and not engine.config.off_autocast:
            engine.scaler.scale(backward_target).backward()
        else:
            backward_target.backward()

        word_count = int(mini_batch[1][1].sum())
...
class SingleTrainer():
    ...
    def __init__(self, target_engine_class, config, device):
